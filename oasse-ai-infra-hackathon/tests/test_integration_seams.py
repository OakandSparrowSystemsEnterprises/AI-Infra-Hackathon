...
def proposal(**overrides):
    # Explicit test proposals replace the old adapter's invented motion defaults.
    return {"action_type": "pick_place", "object_id": "cube-1", "target_bin": "accept",
            "speed_mps": 0.20, "trajectory": [[0.,0.,0.],[0.2,0.,0.1]], **overrides}


class StaticFrameSource:
    def __init__(self, frame): self.frame = frame
    def capture(self): return self.frame
...
class IntegrationSeamTests(unittest.TestCase):
    ...
    def test_openvino_provider_normalizes_evidence(self):
        ...
        def infer(_data):
            return {"confidence": FakeScalar(.97), "workspace_clear": True,
                    "anomaly_score": FakeScalar(.91), "target_label": "defective_cube",
                    "anomaly_bbox_xyxy": FakeArray([.1,.2,.4,.5]),
                    "object_pose_xyzrpy": FakeArray([.2,0.,.03,0.,0.,0.]),
                    "object_dimensions_xyz": [.04,.04,.04], "metadata": {"model": "anomaly-detector"}}
        ...
        self.assertEqual(evidence.anomaly_bbox_xyxy, [.1,.2,.4,.5])
        self.assertEqual(evidence.object_pose_xyzrpy, [.2,0.,.03,0.,0.,0.])
    ...
    def test_openvino_provider_preserves_capture_time_for_freshness(self):
        captured = int(time.time()*1000)-5000
        provider = OpenVINOPerceptionProvider(StaticFrameSource(CapturedFrame(b"stale-frame", captured)),
            lambda _: {"confidence": .99, "workspace_clear": True, "anomaly_score": 0.0})
        self.assertEqual(provider.observe().captured_at_ms, captured)

    def test_openvino_provider_rejects_nonfinite_output(self):
        provider = OpenVINOPerceptionProvider(StaticFrameSource(CapturedFrame(b"frame", int(time.time()*1000))),
            lambda _: {"confidence": float("nan"), "workspace_clear": True, "anomaly_score": 0.0})
        with self.assertRaises(ValueError): provider.observe()

    def test_lerobot_provider_binds_proposal_to_exact_evidence(self):
        evidence = EvidenceFrame.fresh(frame_hash="abc123")
        provider = LeRobotVLAProvider(lambda _: proposal(target_bin="reject",speed_mps=.25,
            trajectory=[[0.,0.,0.],[.1,.2,.3]]), model_name="smolvla")
    ...
    def test_mujoco_actuator_receives_only_authorized_action(self):
        ...
        def execute(action):
            calls.append(action)
            return {"status": "EXECUTED", "sim_state": "advanced"}
        evidence=EvidenceFrame.fresh(confidence=.99,workspace_clear=True,anomaly_score=0.,frame_hash="fresh")
        orch=PhysicalAIOrchestrator(authority=ReferenceAuthorityEngine(),perception=FixedPerception(evidence),
    ...
    def test_stale_real_evidence_never_reaches_mujoco(self):
        calls=[]
        stale=EvidenceFrame.fresh(captured_at_ms=int(time.time()*1000)-5000,
            confidence=.99,workspace_clear=True,frame_hash="stale")
        orch=PhysicalAIOrchestrator(authority=ReferenceAuthorityEngine(evidence_max_age_ms=500),
            perception=FixedPerception(stale),vla=LeRobotVLAProvider(lambda _: proposal()),
    ...
    def test_perception_and_vla_are_constructor_injectable(self):
        perception=FixedPerception(EvidenceFrame.fresh(frame_hash="injected"))
        vla=LeRobotVLAProvider(lambda _: proposal())
        actuator=MuJoCoActuator(lambda _: {"status":"EXECUTED"})
        orch=PhysicalAIOrchestrator(authority=ReferenceAuthorityEngine(),perception=perception,vla=vla,actuator=actuator)
