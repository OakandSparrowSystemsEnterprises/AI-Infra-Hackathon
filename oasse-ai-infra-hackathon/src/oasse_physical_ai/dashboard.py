"""Presentation separated from the API and execution controller."""
...
<div class="flow"><div class="node">Evidence</div><div class="node">Perception</div><div class="node">VLA Proposal</div><div class="node authority">GATEKEEPER<br>ALLOW · TRANSFORM · HOLD · DENY</div><div class="node">Controlled Actuator</div></div>
<div class="buttons">
...
<script>
async function run(s){const st=document.getElementById('status');st.textContent='Evaluating';try{const r=await fetch('/v1/demo/'+s,{method:'POST'});const j=await r.json();if(!r.ok)throw new Error(j.detail||'Request failed');st.textContent=j.decision.verdict+' · '+j.decision.reason_codes.join(', ');document.getElementById('latency').textContent='Authority '+j.decision.authority_latency_ms.toFixed(3)+' ms · total '+j.total_latency_ms.toFixed(3)+' ms · attempted '+j.dispatch_attempted+' · confirmed '+j.executed;document.getElementById('out').textContent=JSON.stringify(j,null,2);}catch(e){st.textContent='Not executed: '+e.message;document.getElementById('latency').textContent='';}}
</script></body></html>'''
