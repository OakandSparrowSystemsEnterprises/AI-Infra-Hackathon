...
def endpoint_url(value: str, *, allow_loopback_http: bool = False) -> str:
    ...
    _ = url.port  # Validate a malformed/non-integer port without making a request.
    return value.rstrip("/")
...
def probe_gatekeeper(url: str, *, expected_policy_version: str, token: str = "",
    ...
    """Exercise the declared demo policy contract, never an actuator.

    The expected policy is deployment configuration, not proof of service
    identity. A passing probe establishes response compatibility only.
    """
    ...
    # Do not expose a token even if a misbehaving service echoes it in a field.
    def redacted(value):
