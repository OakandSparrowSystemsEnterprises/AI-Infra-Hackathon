...
AUTHORITY_RESPONSE_INVALID = "AUTHORITY_RESPONSE_INVALID"  # body is not JSON or lacks required fields
...
def _transformed_action(original: ProposedAction, raw: Any) -> Tuple[Optional[ProposedAction], Optional[str]]:
    """Build the action a TRANSFORM verdict authorizes from the service payload.

    `raw` may be a full action object or only the changed fields. Returns
    (action, None) when it is executable, or (None, reason_code) when it is
    not. The original proposal is never returned as the authorized action.
    """
    if raw is None:
        return None, AUTHORIZED_ACTION_MISSING
    if not isinstance(raw, dict):
        return None, AUTHORIZED_ACTION_INVALID
    overrides = {key: value for key, value in raw.items() if key in _ACTION_FIELDS}
    try:
        if "speed_mps" in overrides:
            speed = float(overrides["speed_mps"])
            if not math.isfinite(speed) or speed < 0:
                return None, AUTHORIZED_ACTION_INVALID
            overrides["speed_mps"] = speed
        candidate = replace(original, **overrides)
    except (TypeError, ValueError):
        return None, AUTHORIZED_ACTION_INVALID
    if any(getattr(candidate, name) != getattr(original, name) for name in _BOUND_FIELDS):
        return None, AUTHORIZED_ACTION_BINDING_MISMATCH
    if candidate == original:
        return None, AUTHORIZED_ACTION_UNCHANGED
...
class GatekeeperClient:
    ...
    The adapter fails closed. Transport errors, timeouts, non-2xx statuses,
    and malformed bodies become HOLD decisions carrying a reason code instead
    of exceptions, so the orchestrator still seals a decision receipt and the
    actuator is never reached. A TRANSFORM verdict executes only the
    authorized action returned by the service; it never falls back to the
    original proposal.
    """
    ...
    def evaluate(self, evidence: EvidenceFrame, action: ProposedAction) -> AuthorityDecision:
        ...
        try:
            ...
            log.warning("Gatekeeper response is not JSON (%s) for action %s; holding", type(exc).__name__, action.action_id)
            return self._hold(evidence, action, [AUTHORITY_RESPONSE_INVALID], start)
        try:
            return self._decision_from(data, evidence, action, start)
        except (AttributeError, KeyError, TypeError, ValueError) as exc:
            log.warning("Gatekeeper response malformed (%s) for action %s; holding", type(exc).__name__, action.action_id)
            return self._hold(evidence, action, [AUTHORITY_RESPONSE_INVALID], start)

    def _decision_from(self, data: Any, evidence: EvidenceFrame, action: ProposedAction, start: int) -> AuthorityDecision:
        ...
        reasons: List[str] = [str(code) for code in (data.get("reason_codes") or [])]
        evaluated_at_ms = int(data.get("evaluated_at_ms") or int(time.time() * 1000))
        latency = float(data.get("authority_latency_ms") or 0.0)
        policy_version = str(data.get("policy_version") or LIVE_POLICY_VERSION_DEFAULT)

        authorized: Optional[ProposedAction] = None
        if verdict == Verdict.ALLOW:
            authorized = action
        elif verdict == Verdict.TRANSFORM:
            authorized, problem = _transformed_action(action, data.get("authorized_action"))
            if problem is not None:
                log.warning("Gatekeeper TRANSFORM for action %s is not executable (%s); holding", action.action_id, problem)
                return self._hold(
                    evidence,
                    action,
                    reasons + [problem],
