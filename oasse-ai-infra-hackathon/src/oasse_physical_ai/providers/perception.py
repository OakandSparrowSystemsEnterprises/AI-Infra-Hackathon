...
class MockPerceptionProvider:
    ...
    def observe(self, scenario: str = "allow") -> EvidenceFrame:
        ...
        data = f"mock-frame:{scenario}:{now}:{self._sequence}".encode()
        base = EvidenceFrame.fresh(
            captured_at_ms=now, confidence=0.98, workspace_clear=True,
            anomaly_score=0.02, target_label="normal_cube", frame_hash=frame_sha256(data),
            object_pose_xyzrpy=[0.20, 0.00, 0.03, 0.0, 0.0, 0.0],
            object_dimensions_xyz=[0.04, 0.04, 0.04], frame_sequence=self._sequence,
            metadata={"provider": "mock-perception"},
        )
        if scenario == "stale":
            return replace(base, captured_at_ms=now - 2000)
